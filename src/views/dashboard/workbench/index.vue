        ...
        <div class="page">
            <div class="page__header">
                <search-bar bordered @onSearch="onHandleSearch" @onReset="onHandleReset" :action="false" customAdd>
                    <template #right>
                        ...
                        <search-bar-label width="280" label="运营商">
                            <a-select
                        ...
                        <search-bar-label label="游戏" width="280">
                            <a-select
                        ...
                        <search-bar-label label="货币" width="280">
                            <a-select
            ...
            <div class="page__container">
                ...
                >
                    ...
                    <vxe-column field="agentId" title="运营商ID" min-width="150" />
...
<script setup>
import { reactive, ref, getCurrentInstance, watch, onMounted, computed, nextTick } from 'vue'
import useTable from '@/hooks/useTable'
import { useRollNumber } from '@/hooks/useRollNumber'
import { aggregateData, statisticsData } from '@/api/homeApi.js'
import { gamelistAuth, agentListAuth, listAuthProvider } from '@/api/merchant.js'
...
const { loading, queryParams, tableData, pagerInfo, total, handleSearch, handleReset, reloadData } = useTable(
    (params) =>
        statisticsData({
            ...
            agentId: params.agentId?.map((id) => Number(id)) || [],
...
const loadAgentList = async () => {
    const resp = await agentListAuth()
...
const onHandleSearch = () => {
    if ((!dateValue.value || dateValue.value === '') && (!timeValue.value || timeValue.value.length === 0)) {
        proxy.$message.warning('请至少选择日期或时间！')
        return
    }
    handleSearch()
}
